...
listing = dir('../../data/process_recovery_tpt');

tbl = readtable([ '../../data/process_recovery_tpt/' listing(1+2).name]);
...
for l = 1:(length(listing)-2)
    ...
    tbl = readtable([ '../../data/process_recovery_tpt/' listing(l+2).name]);
    %get the parameters of the simulation
    temp = sscanf(listing(l+2).name, 'process_recovery_tpt(N=%d,minsamplesize=%d,prelec_gamma=%g,theta=%g)_iteration=%d'); 
    NSim(l) = temp(1); 
